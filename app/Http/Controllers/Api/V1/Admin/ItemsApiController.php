...
use App\Models\Modern\{Item, ItemType, ItemDate, ItemFeatures, Currency, ItemVehicle, ItemMeta};
...
class ItemsApiController extends Controller
{
    ...
    public function getItemDetails(Request $request)
    {
        ...
        try {
            ...
            $itemDetail->save();
            return $this->addSuccessResponse(200,trans('front.Result_found'), ['ItemDetails' => $itemData]);
    ...
    public function insertItem(Request $request)
    {
        ...
        return $this->addSuccessResponse(200,trans('front.item_added'), $Item);
        try {
        } catch (\Exception $e) {
    ...
    public function editItem(Request $request)
    {
        ...
        $item->update([
            ...
            'booking_policies_id' => $request->booking_policies_id

        ]);
        if ($request->input('availability_dates'))
            ItemDate::where('item_id', $item->id)->where('booking_id', '=', 0)
                ->where('date', '>=', now()->toDateString()) // Delete future entries
                ->delete();
        ...
        $discountData = [
            ...
            'weekly_discount_type' => $request->weekly_discount_type,
            'monthly_discount' => $request->monthly_discount,
            'monthly_discount_type' => $request->monthly_discount_type,
        ];
        $this->addOrUpdateItemMeta($item->id, $discountData);

        // Fetch existing item meta or create new if not exists
        $itemMeta = $item->itemMeta->first();
        ...
        return $this->addSuccessResponse(200,trans('front.item_updated_successfully'), $item);
    ...
    public function deleteItem(Request $request)
    {
        try {
            ...
            if (!$item) {
                return $this->errorResponse(404,trans('front.item_not_found'));
            }
            ...
            return $this->addSuccessResponse(200,trans('front.item_deleted_successfully'), []);
        } catch (\Exception $e) {
            return $this->addErrorResponse(500,trans('front.something_wrong'), $e->getMessage());
        }
    }
    ...
    public function deletefrontimage(Request $request)
    {
        ...
        try {
            ...
            if ($item->front_image) {
                ...
                return $this->addErrorResponse(500,trans('front.please_upload_image_after_delete'), '');
            }
            return $this->addSuccessResponse(200,trans('front.Deleted_successful '), "");
        } catch (\Exception $e) {
            // update, return a generic error response
            return response()->json([
                'message' =>trans('front.Failed_to_delete_front_image_Please_try_again_later'),
            ], 500);
        }
    }
    ...
    public function deletegalleryimage(Request $request)
    {
        ...
        try {
            ...
            // update, return a generic error response
            return response()->json([
                'message' =>trans('front.Failed_to_delete_gallery_image_Please_try_again_later'),
            ], 500);
        }
    }
    ...
    public function myItems(Request $request)
    {
        ...
        try {
            ...
            $hostStatus = $appuser ? $appuser->host_status : null;
            ...
            $itemsQuery = Item::where('userid_id', $user_id)
                ->where('module', $module);
            // Add search filter
            ...
            $itemAll = $itemsQuery->skip($offset)->take($limit)->get();
            $itemIds = $itemAll->pluck('id');

            $itemMetas = ItemMeta::whereIn('rental_item_id', $itemIds)
                ->get()
                ->groupBy('rental_item_id')
                ->mapWithKeys(function ($metas, $itemId) {
                    return [
                        $itemId => $metas->pluck('meta_value', 'meta_key')
                    ];
                });


            $item = $itemsQuery->skip($offset)->take($limit)->get()->map(function ($item) use ($module, $itemMetas) {


                $itemType = ItemType::find($item->item_type_id);
                $item['item_type'] = $itemType ? $itemType->name : '';
                ...
                // 
                $itemId = $item->id;
                $itemDates = ItemDate::where('item_id', $itemId)->where('date', '>=', now())->orderBy('date', 'asc')->get();
                ...
                foreach ($itemDates as $date) {
                    if ($date->status === 'Available') {
                        $availableDates[$date->range_index][] = [
                            'date' => $date->date,
                            'price' => $date->price,
                        ];
                ...
                $availableDatesString = json_encode($availableDates);
                $item['metaData'] = $this->returnItemMetaData($item->id, $module);
                // Add Available and Not Available date information to the item data
                $item['available_dates'] = $availableDatesString;
                $item['not_available_dates'] = $notAvailableDates;
                $item['booked_dates'] = $booked_dates;
                $metaDataDiscount = $itemMetas->get($itemId, collect());

                // Add meta data to the item
                $item['weekly_discount'] =  $metaDataDiscount->get('weekly_discount'); // Default value is 0
                $item['monthly_discount'] =  $metaDataDiscount->get('monthly_discount'); // Default value is 0
                $item['weekly_discount_type'] =  $metaDataDiscount->get('weekly_discount_type'); // Default is 'fixed'
                $item['monthly_discount_type'] =  $metaDataDiscount->get('monthly_discount_type'); // Default is 'fixed'
                $item['item_info'] =  $metaDataDiscount->get('itemMetaInfo');

                return $item;
            });;

            $nextOffset = $offset + count($item);

            if ($item->isEmpty()) {
                $nextOffset = -1;
            }

            $hasItems = $item->isNotEmpty();

            $hiddenFields = ['is_verified', 'is_featured', 'created_at', 'updated_at', 'deleted_at', 'userid_id', 'views_count', 'media'];

            // Make items hidden
            $item = $item->makeHidden($hiddenFields);
    ...
    public function itemSearch(Request $request)
    {
        ...
        return $this->addSuccessResponse(200,trans('front.itemType_found'), [
    ...
    public function addEditItemImage(Request $request)
    {
        ...
        try {
            ...
            if ($request->has('front_image_doc') && $request->input('front_image_doc')) {


                if ($item->hasMedia('front_image_doc')) {
                    $item->getFirstMedia('front_image_doc')->delete();
                }


                $frontImageDoc = $request->input('front_image_doc');
                $frontImageDocUrl = $this->serveBase64Image($frontImageDoc);
                $item->addMedia($frontImageDocUrl)->toMediaCollection('front_image_doc');
            ...
            return $this->addSuccessResponse(200,trans('front.images_added_successfully'), $item);
        } catch (\Exception $e) {
            // update, return a generic error response
            return $this->addErrorResponse(500,trans('front.something_wrong'), $e->getMessage());
        }
