...
use App\Models\{Booking, Transaction, AppUser, GeneralSetting};
use App\Http\Controllers\Traits\{VendorWalletTrait, NotificationTrait};
use App\Models\Modern\{Item};
use Carbon\Carbon;

trait PaymentStatusUpdaterTrait
{
    use VendorWalletTrait, NotificationTrait;
    public function updateBookingStatus($orderID, $responceData)
    {
        ...
        if ($booking) 
            ...
            $transaction->save();
           // if(isset($responceData->payment_status) && $responceData->payment_status === 'paid')
            if (
                (isset($responceData->payment_status) && $responceData->payment_status === 'paid')
                || (isset($responceData->gateway_name) && $responceData->gateway_name === 'razorpay')
                )
            {
            ...
            if (($responceData->payment_status === 'paid') || ($responceData->gateway_name === 'razorpay')) 
            {
            $template_id = 14;
            $valuesArray = array();
            ...
            return json_encode(['status' => 'failed', 'message' => 'Payment failed']);
