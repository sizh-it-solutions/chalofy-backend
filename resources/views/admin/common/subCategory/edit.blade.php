        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route($updateRoute, [$subCatData->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('model_name') ? 'has-error' : '' }}">
                            <label class="required" for="model_name">{{ trans('global.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $subCatData->name) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('make_id') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="make_id" id="make_id" required>
                                ...
                                @foreach($mainCategory as $mainCategoryData)
                                    <option value="{{ $mainCategoryData->id }}" {{ $subCatData->make_id == $mainCategoryData->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="description" id="description" value="{{ old('description', $subCatData->description) }}">
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status" required>
                                ...
                                @foreach(App\Models\SubCategory::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $subCatData->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
...
<script>
    ...
    init: function () {
@if(isset($subCatData) && $subCatData->image)
      var file = {!! json_encode($subCatData->image) !!}
