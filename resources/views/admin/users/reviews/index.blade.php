            ...
            <form class="form-horizontal" enctype="multipart/form-data" action="" method="GET" accept-charset="UTF-8" id="bookingFilterForm">
                ...
                <div class="col-md-12">
                    <div class="row">
                        ...
                        <div class="col-md-3 col-sm-12 col-xs-12">
                            <label>{{ trans('global.vehicle') }}</label>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class=" panel-default">
                    ...
                    {{ trans('global.vehicle') }} {{trans('global.Review')}} {{ trans('global.list') }}
                    </div>
                            ...
                            <table class=" table table-bordered table-striped table-hover datatable datatable-Booking">
                                ...
                                <tbody>
                                        ...
                                        <tr data-entry-id="{{ $reviewData->id }}">
                                            ...
                                            <td>
                                                @if ($reviewData->item)
                                                    @switch($reviewData->module)
                                                        @case(2)
                                                            <a target="_blank" href="{{ route('admin.vehicles.base', $reviewData->item_id) }}">
                                                                {{ $reviewData->item->title ?? $reviewData->item_name }}
                                                            </a>
                                                            @break
                                                        @default
                                                            {{ $reviewData->item->title ?? $reviewData->item_name }}
                                    ...
                                    @php
                                        ...
                                        $routeName = 'admin.overviewprofile'; 
                                    @endphp
                                            <td>
                                                @if ($reviewData->guest)
                                            
                                                    <a target="_blank" href="{{ route($routeName, $reviewData->guestid) }}?user_type={{ $reviewData->guest->user_type }}">
                                                        {{ $reviewData->guest->first_name ?? $reviewData->guest_name }} {{ $reviewData->guest->last_name ?? '' }}
                                                    </a>
                                                @else
                                            ...
                                            <td>
                                                @if ($reviewData->host)
                                                    ...
                                                    <a target="_blank" href="{{ route($routeName, $reviewData->hostid) }}?user_type={{ $reviewData->host->user_type }}">
                                                        {{ $reviewData->host->first_name ?? $reviewData->host_name }} {{ $reviewData->host->last_name ?? '' }}
                                                    </a>
                                                @else
                                            ...
                                            <td>
                                                @can('review_edit')
                                                    <a class="btn btn-xs btn-info" href="{{ route('admin.reviews.edit', $reviewData->id) }}">
